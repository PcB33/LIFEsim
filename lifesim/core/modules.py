...
class ExtractionModule(Module):
    '''
    Module for extracting planetary parameters from an existing catalog that has already run through a simulation
    '''
    def __init__(self,
                 name: str):
        super().__init__(name=name)
    ...
    #ToDo question: should this be an abstract method?
    @abc.abstractmethod
    def single_spectrum_extraction(self):
        """
        The single_spectrum_extraction function should return the extracted parameters like spectra, snr, sigma, position
        parameters r (radial) and phi (azimuthal), planet radius, planet temperature (and potentially additional ones)
        """
